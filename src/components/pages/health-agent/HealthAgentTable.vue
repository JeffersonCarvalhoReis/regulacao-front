<template>
  ...
  <v-dialog v-model="viewHealthAgentDetails">
    <health-agent-details
      :health-agent-data="healthAgentData"
      @close="viewHealthAgentDetails = false"
    />
  </v-dialog>

  <v-dialog v-model="editHealthAgent" class="z-999">
    <health-agent-form
      :model-value="selectedHealthAgent"
      @close="editHealthAgent = false"
      @save="submit"
    />
  </v-dialog>
</template>
...
const tooltipTextDelete =
  "Não é possível excluir o agente de saúde enquanto houver pacientes vinculados a ele.";
...
const handleDelete = async (healthAgent) => {
  const confirm = await confirmModal(
    `Tem certeza que deseja excluir o Agente Comunitário de Saúde <strong>${healthAgent.name}</strong>?`,
    "Atenção",
  );
  if (confirm) {
