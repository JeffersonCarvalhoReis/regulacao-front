<template>
  ...
  <v-dialog v-model="viewRequestingUnitDetails">
    <requesting-unit-details
      :requesting-unit-data="requestingUnitData"
      @close="viewRequestingUnitDetails = false"
    />
  </v-dialog>

  <v-dialog v-model="editRequestingUnit" class="z-999">
    <requesting-unit-form
      :model-value="selectedRequestingUnit"
      @close="editRequestingUnit = false"
      @save="submit"
    />
  </v-dialog>
</template>
...
const tooltipTextDelete =
  "Não é possível excluir a unidade enquanto houver solicitações vinculadas a ela.";
...
const handleDelete = async (requestingUnit) => {
  const confirm = await confirmModal(
    `Tem certeza que deseja excluir a Unidade <strong>${requestingUnit.name}</strong>?`,
    "Atenção",
  );
  if (confirm) {
