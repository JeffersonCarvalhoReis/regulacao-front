<template>
  <BaseCard :title="title" @close="emit('close')">
    ...
    <div
      ...
      class="mx-auto mb-10 flex flex-col justify-end"
    >
      <v-card-title>
        <hr />
      ...
      <v-card-text>
        <div class="leading-6">
          <div class="flex items-center my-2">
            <img
              alt="Brasao do municipio de itaguaçu"
              class="w-20"
              src="/src/assets/images/brasao.svg"
            />
          ...
          >{{ formatDate(props.appointmentData.solicitation.patient_birth_date)
          }}<br />
          <strong>Celular: </strong
          >{{ props.appointmentData.solicitation.patient_phone }}<br />
          <strong>Endereço: </strong
          ><span class="uppercase"
            >{{ props.appointmentData.solicitation.patient_street }} -
            {{ props.appointmentData.solicitation.patient_neighborhood }}</span
          ><br />
          ...
          <hr />
          <strong>Data: </strong>{{ formatDate(props.appointmentData.date)
          }}<br />
          <strong>Horário: </strong>{{ props.appointmentData.time }}<br />
          <strong>Local: </strong
          ><span class="uppercase">{{
            props.appointmentData.provider_unit
          }}</span>
          <br />
          <div v-if="props.appointmentData.provider_unit_localization">
            <strong>Localização: </strong
            ><span class="uppercase">{{
              props.appointmentData.provider_unit_localization
            }}</span>
            <br />
          </div>
          <hr />
          ...
          <strong v-if="props.appointmentData.doctor">Médico: </strong>
          <span class="uppercase">{{ props.appointmentData.doctor }}</span
          ><br />
          <div v-if="isExam">
            <strong>Procedimento:</strong>
            <span class="uppercase">{{ props.appointmentData.procedure }}</span
            ><br />
          </div>
          <div v-else>
            <strong>Especialidade: </strong>
            <span class="uppercase">{{ props.appointmentData.specialist }}</span
            ><br />
          </div>
          <p>
            <strong>Recomendações:</strong>
            <span class="uppercase whitespace-break-spaces">{{
              recomendation
            }}</span>
          </p>
        </div>
  ...
  <v-dialog v-model="editRecomendationDialog" class="z-990">
    <AppointmentRecomendations
      :recomendation-default="recomendation"
      @close="editRecomendationDialog = false"
      @save="handleSaveRecomendation"
    />
  </v-dialog>
</template>
...
const recomendation = ref(
  "Atenção, OBRIGATÓRIO LEVAR REQUISIÇÃO MÉDICA ORIGINAL!",
);
