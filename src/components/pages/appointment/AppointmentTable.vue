<template>
  <div>
    <appointment-search
      ...
      @search-appointment-status="searchStatus"
    >
      <slot />
    </appointment-search>
    <div class="text-right">
      <v-btn
        ...
        @click="handleExportAppointments"
      >
        Exportar Planilha
      </v-btn>
    </div>
    ...
    >
      ...
      <v-tab class="font-bold px-10" value="exam" v-if="role !== 'caps'">
        Exames
      </v-tab>
    </v-tabs>
  ...
  <v-dialog v-model="editAppointment" class="z-999">
    <appointment-form
      :model-value="selectedAppointment"
      @close="editAppointment = false"
      @save="submit"
    />
  </v-dialog>
...
const confirmAppointment = async (appointment) => {
  const confirm = await confirmModal(
    `Confirmar agendamento de <strong>${appointment.patient}</strong>?`,
    "Confirmação",
    "question",
