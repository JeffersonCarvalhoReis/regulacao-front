  ...
  <base-card :title="title" @close="emit('close')">
    ...
    <div class="mx-10 mb-15">
      <base-table
        ...
        :class-new-action="classRemoveDoctor"
        :edit="false"
        :headers="headers"
        hide-default-footer
        :icon-new-action="iconRemoveDoctor"
        :items="doctorsReversed"
        :loading="loadingList"
        :new-action="true"
        :show-delete="false"
        :text-new-action="textRemoveDoctor"
    ...
    <v-dialog
      ...
      class="z-999"
    >
      <AddDoctors :doctors-data="doctorsToAdd" @close="dialogAddDoctors = false" @save="addDoctor" />
    </v-dialog>
...
<script setup>
  ...
  const { data, loadingList, refetch, setInclude, setFilter, update } = useSpecialistApi();
  ...
  const handleRemoveDoctor = async doctor => {
    const confirm = await confirmModal(`Tem certeza que deseja remover o médico <strong>${doctor.name}</strong> da Especialidade <strong>${props.modelValue.name}</strong>?`, 'Atenção');
    if(confirm) {
