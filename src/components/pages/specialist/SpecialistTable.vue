<template>
  ...
  <v-dialog v-model="viewSpecialistDetails">
    <specialist-details
      :specialist-data="specialistData"
      @close="viewSpecialistDetails = false"
    />
  </v-dialog>

  <v-dialog v-model="editSpecialist" class="z-999">
    <specialist-form
      :model-value="selectedSpecialist"
      @close="editSpecialist = false"
      @save="submit"
    />
  </v-dialog>

  <v-dialog v-model="dialogDoctorManager" class="z-999">
    <specialist-doctor-manager
      :model-value="selectedSpecialist"
...
const tooltipTextDelete =
  "Não é possível excluir a especialidade enquanto houver solicitações vinculadas a ela.";
...
const handleDelete = async (specialist) => {
  const confirm = await confirmModal(
    `Tem certeza que deseja excluir a Especialidade <strong>${specialist.name}</strong>?`,
    "Atenção",
  );
  if (confirm) {
