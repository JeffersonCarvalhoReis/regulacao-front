      ...
      <div ref="printSection" class="w-4xl flex flex-col items-center">
        ...
        <div class="flex flex-col w-full gap-1 py-4">
          <LabeledLineGroup>
            ...
            <LabeledLine
              label="CPF"
              :value="appointmentData.solicitation.patient_cpf"
            />
          </LabeledLineGroup>
          <LabeledLineGroup>
            <LabeledLine
              label="Endereço"
              :value="`${appointmentData.solicitation.patient_street} - ${appointmentData.solicitation.patient_neighborhood}`"
            />
          </LabeledLineGroup>
          <LabeledLineGroup>
            <LabeledLine
              label="Nome da Mãe"
              :value="appointmentData.solicitation.patient_mother_name"
            />
          </LabeledLineGroup>
          <LabeledLineGroup>
            <LabeledLine label="Responsável" :value="''" />
            <!-- Valor vazio adicionado -->
          </LabeledLineGroup>
          <LabeledLineGroup>
            <LabeledLine
              label="Data de Nasc."
              :value="
                formatDate(appointmentData.solicitation.patient_birth_date)
              "
            />
            <LabeledLine
              label="Idade"
              :value="`${calculateAge(appointmentData.solicitation.patient_birth_date)}`"
            />
            <LabeledLine
              label="Gênero"
              :value="genderMap(appointmentData.solicitation.patient_gender)"
            />
          </LabeledLineGroup>
          <LabeledLineGroup>
            <LabeledLine
              label="Cor"
              :value="appointmentData.solicitation.patient_race"
            />
            <!-- Valor vazio adicionado -->
            <LabeledLine label="Estado Civil" :value="''" />
            <!-- Valor vazio adicionado -->
            <LabeledLine label="Profissão" :value="''" />
            <!-- Valor vazio adicionado -->
          </LabeledLineGroup>
          <LabeledLineGroup>
            <LabeledLine
              label="Telefone"
              :value="appointmentData.solicitation.patient_phone"
            />
            <LabeledLine
              label="ACS"
              :value="appointmentData.solicitation.patient_acs"
            />
            <LabeledLine
              label="Unidade de Saúde"
              :value="appointmentData.solicitation.patient_health_unit"
            />
          </LabeledLineGroup>
          <LabeledLineGroup>
            <LabeledLine label="Médico" :value="appointmentData.doctor" />
            <LabeledLine
              v-if="appointmentData.solicitation.solicitation_type == 'exam'"
              label="Procedimento"
              :value="appointmentData?.procedure"
            />
            <LabeledLine
              v-if="
                appointmentData.solicitation.solicitation_type == 'consultation'
              "
              label="Consulta"
              :value="appointmentData?.specialist"
            />
          </LabeledLineGroup>
          <LabeledLineGroup>
            <LabeledLine
        ...
        <div class="border border-gray-500 p-0 w-full flex flex-col">
          <div
            class="p-2 bg-blue-300 w-full flex justify-center border-b border-gray-500"
          >
