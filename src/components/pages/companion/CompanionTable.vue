<template>
  ...
  <v-dialog v-model="viewCompanionProfile" class="z-900">
    <companion-profile
      :companion-data="selectedCompanion"
      @close="viewCompanionProfile = false"
    />
  </v-dialog>

  <v-dialog v-model="editCompanion" class="z-999">
    <companion-form
      :model-value="selectedCompanion"
      @close="editCompanion = false"
      @save="submit"
    />
  </v-dialog>
</template>
...
const tooltipTextDelete =
  "Não é possível excluir essa pessoa enquanto houver viagens TFD vinculadas a ela.";
...
const handleDelete = async (compnion) => {
  const confirm = await confirmModal(
    `Tem certeza que deseja excluir o acompanhante <strong>${compnion.name}</strong>?`,
    "Atenção",
  );
  if (confirm) {
