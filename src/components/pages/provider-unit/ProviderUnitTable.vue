<template>
  ...
  <v-dialog v-model="viewProviderUnitDetails">
    <provider-unit-details
      :provider-unit-data="providerUnitData"
      @close="viewProviderUnitDetails = false"
    />
  </v-dialog>

  <v-dialog v-model="editProviderUnit" class="z-999">
    <provider-unit-form
      :model-value="selectedProviderUnit"
      @close="editProviderUnit = false"
      @save="submit"
    />
  </v-dialog>

  <v-dialog v-model="dialogDoctorManager" class="z-999">
    <provider-unit-doctor-manager
      :model-value="selectedProviderUnit"
      @close="dialogDoctorManager = false"
      @save="submitUpdateDoctors"
...
const tooltipTextDelete =
  "Não é possível excluir a unidade prestadora enquanto houver solicitações vinculadas a ela.";
...
const handleDelete = async (providerUnit) => {
  const confirm = await confirmModal(
    `Tem certeza que deseja excluir a Unidade Prestadora <strong>${providerUnit.name}</strong>?`,
    "Atenção",
  );
  if (confirm) {
