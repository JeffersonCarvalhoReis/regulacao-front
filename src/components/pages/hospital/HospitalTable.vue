<template>
  ...
  <v-dialog v-model="viewHospitalDetails">
    <hospital-details
      ...
      @close="viewHospitalDetails = false"
    />
  </v-dialog>

  <v-dialog v-model="editHospital" class="z-999">
    <hospital-form
      :model-value="selectedHospital"
      @close="editHospital = false"
      @save="submit"
    />
  </v-dialog>
</template>
...
const handleDelete = async (hospital) => {
  const confirm = await confirmModal(
    `Tem certeza que deseja excluir o hospital <strong>${hospital.name}</strong>?`,
    "Atenção",
  );
  if (confirm) {
