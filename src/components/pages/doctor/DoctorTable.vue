<template>
  ...
  <v-dialog v-model="viewDoctorDetails">
    <doctor-details
      ...
      @close="viewDoctorDetails = false"
    />
  </v-dialog>
  <v-dialog v-model="editDoctor" class="z-999">
    <doctor-edit
      :model-value="selectedDoctor"
      @close="editDoctor = false"
      @save="submit"
      @update-table="refetch"
    />
  </v-dialog>
</template>
...
const tooltipTextDelete =
  "Não é possível excluir médico enquanto houver unidades vinculadas a ele.";
...
const submit = async (value) => {
  try {
    ...
    await refetch();
    const refreshedDoctor = data.value.find(
