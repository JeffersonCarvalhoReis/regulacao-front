<template>
  <BaseCard title="Trocar Data da Viagem do Paciente" @close="emit('close')">
    <v-card-text>
      <BaseSection class="mb-4">
        <InfoGroup title="Dados do Paciente">
          <div>Nome: {{ props.patientData.name }}</div>
          <div>
            Data de Nascimento: {{ formatDate(props.patientData.birth_date) }}
          </div>
          ...
          <div>
            Endereço:
            {{
              `${props.patientData.street} - ${props.patientData.neighborhood}`
            }}
          </div>
        </InfoGroup>
        <InfoGroup v-if="hasCompanion" title="Dados do Acompanhante">
          <div>Nome: {{ props.patientData.companion.name }}</div>
          <div v-if="props.patientData.kinship">
            Parentesco: {{ props.patientData.kinship }}
          </div>
          <div>
            Data de Nascimento:
            {{ formatDate(props.patientData.companion.birth_date) }}
          </div>
          <div>
          ...
          <div>
            Endereço:
            {{
              `${props.patientData.companion.street} - ${props.patientData.companion.neighborhood}`
            }}
          </div>
        </InfoGroup>
        <InfoGroup title="Dados da Consulta">
          ...
          <div>
            Data da Consulta:
            {{ formatDate(props.patientData.appointment_date) }}
          </div>
          <div>
            Horário da Consulta: {{ props.patientData.appointment_time }}
          </div>
          <div v-if="props.patientData.notes">
            Motivo: <span class="uppercase">{{ props.patientData.notes }}</span>
          </div>
          <div>
            Prioridade: {{ booleanToLabel(props.patientData.is_priority) }}
          </div>
        </InfoGroup>
      </BaseSection>
    </v-card-text>
...
const hasCompanion = computed(() => !!props.patientData.companion?.id);
...
const handleMovePassenger = async () => {
  const confirm = await confirmModal(
    `Confirmar a alteração de data do paciente <strong>${props.patientData.name}</strong>?`,
    "Atenção",
  );
  if (confirm) {
    onSubmit();
