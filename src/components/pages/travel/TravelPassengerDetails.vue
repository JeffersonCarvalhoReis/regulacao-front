    ...
    <v-card-text class="flex flex-col gap-2">
      <BaseSection>
        <InfoGroup title="Dados do Paciente">
          ...
          <div>
            Endereço: {{ `${props.data.street} - ${props.data.neighborhood}` }}
          </div>
        </InfoGroup>
      </BaseSection>
      <BaseSection v-if="hasCompanion">
        <InfoGroup title="Dados do Acompanhante">
          <div>Nome: {{ props.data.companion?.name }}</div>
          <div v-if="props.data.kinship">
            Parentesco: {{ props.data.kinship }}
          </div>
          <div>
            Data de Nascimento:
            {{ formatDate(props.data.companion?.birth_date) }}
          </div>
          ...
          <div>
            Endereço:
            {{
              `${props.data.companion?.street} - ${props.data.companion?.neighborhood}`
            }}
          </div>
        </InfoGroup>
      </BaseSection>
      <BaseSection v-for="(extra, i) in props.data.extra_companions" :key="i">
        <InfoGroup title="Dados do Acompanhante">
          ...
          <div>
            Data de Nascimento: {{ formatDate(extra.companion.birth_date) }}
          </div>
          ...
          <div>
            Endereço:
            {{ `${extra.companion.street} - ${extra.companion.neighborhood}` }}
          </div>
        </InfoGroup>
      </BaseSection>
      <BaseSection>
        <InfoGroup title="Dados da Consulta">
          ...
          <div>
            Data da Consulta: {{ formatDate(props.data.appointment_date) }}
          </div>
          <div>Horário da Consulta: {{ props.data.appointment_time }}</div>
          <div v-if="props.data.notes">
            Motivo: <span class="uppercase">{{ props.data.notes }}</span>
          </div>
...
const hasCompanion = computed(() => !!props.data.companion?.id);
const title = computed(() =>
  hasCompanion.value
    ? "Detalhes do Paciente e Acompanhante"
