<template>
  ...
  <v-dialog v-model="viewHealthUnitDetails">
    <health-unit-details
      :health-unit-data="healthUnitData"
      @close="viewHealthUnitDetails = false"
    />
  </v-dialog>

  <v-dialog v-model="editHealthUnit" class="z-999">
    <health-unit-form
      :model-value="selectedHealthUnit"
      @close="editHealthUnit = false"
      @save="submit"
    />
  </v-dialog>
</template>
...
const tooltipTextDelete =
  "Não é possível excluir a unidade de saúde enquanto houver agentes de saúde vinculados a ela.";
...
const handleDelete = async (healthUnit) => {
  const confirm = await confirmModal(
    `Tem certeza que deseja excluir a Unidade de Saúde <strong>${healthUnit.name}</strong>?`,
    "Atenção",
  );
  if (confirm) {
