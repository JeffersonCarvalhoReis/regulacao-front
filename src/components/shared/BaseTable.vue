      ...
      <template #item.action="{ item }">
        ...
        <template v-else>
          <v-btn-group divided variant="outlined">
              ...
              <v-tooltip v-if="newAction" :text="textNewAction">
                <template #activator="{ props }">
                  <v-btn
                    ...
                    :class="
                      currentStatus(item)
                        ? 'text-slate-500 border-0 ml-1 h-full'
                        : classNewAction
                    "
                  ...
                  >
                    <v-icon>{{
                      currentStatus(item) ? "mdi-account-off" : iconNewAction
                    }}</v-icon>
                  </v-btn>
                </template>
              </v-tooltip>
              ...
              <template v-if="edit">
                <span>
                  <v-btn
                    ...
                    v-bind="props"
                    :class="
                      item.deletable || deletable || editOn
                        ? classEdit
                        : 'text-slate-500 border-0  ml-1 h-full'
                    "
            ...
            <v-tooltip v-if="register" text="Cadastrar">
              <template #activator="{ props }">
                <v-btn
                  v-bind="props"
            ...
            <template v-if="showDelete">
              <span>
                <v-btn
                  ...
                  :class="
                    deleteOn && (item.deletable || deletable)
                      ? chooseClassDelete(item?.status)
                      : 'text-slate-500 border-0 ml-1 h-full'
                  "
                  :disabled="!(deleteOn && (item.deletable || deletable))"
                  icon
                  @click="selectItemDelete(item)"
                >
                  <v-icon>{{
                    deleteOn && (item.deletable || deletable)
                      ? chooseIconDelete(item?.status, iconDelete)
                      : "mdi-delete-off"
                  }}</v-icon>
                </v-btn>
                ...
                <v-tooltip activator="parent">
                  {{
                    deleteOn && (item.deletable || deletable)
                      ? chooseTextDelete(item?.status, textDelete)
                      : tooltipTextDelete
                  }}
                </v-tooltip>
              </span>
        ...
        <v-container fluid>
          <v-row align="center" justify="center">
            <v-col class="text-center" cols="12">
              <v-icon color="grey" size="large"> mdi-database-off </v-icon>
...
const props = defineProps({
  ...
  classEdit: {
    type: String,
    default: "text-ita-blue bg-white/0/0 border-0 ml-1 h-full",
  },
  ...
  tooltipTextDelete: {
    type: String,
    default: "Não é possivel excluir pois há outros registros vinculados",
  },
  deletable: { type: Boolean, default: false },
  hideDefaultFooter: { type: Boolean, default: false },
  editOn: { type: Boolean, default: true },
  deleteOn: { type: Boolean, default: true },
